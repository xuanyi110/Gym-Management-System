module.exports = {
    ...
    getLockerwithCustomer: 'SELECT l.id, l.locker_size, l.customer_id, c.first_name, c.last_name from customer c, locker l where l.customer_id = c.id',
    ...
    getCalByMealPlan: 'SELECT mp.id, mp.name, SUM(f.calories) from food_item f, food_item_in_meal_plan fm, meal_plan mp where fm.food_item = f.name and mp.id = fm.plan_id and mp.id = "{{meal_plan_ID}}" group by mp.id',
}