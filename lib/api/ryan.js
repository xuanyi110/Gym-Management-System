module.exports = {
  ...
  getCustCoaches:
    'SELECT s.id, s.firstName, s.lastName, c.year_of_exp, ce.coach_level ' +
    'FROM coach c, staff s, course_registration cr, gym_session gs, coach_experience ce ' +
    'WHERE cr.customer_id = {{cust_id}} AND cr.session_number = gs.session_number AND gs.coach_id = c.staff_id ' +
    'AND c.staff_id = s.id AND c.year_of_exp = ce.years',
  getNumTuples: 'SELECT COUNT(*) FROM {{table_name}}',
  getAvgSalaryByGender:
    "SELECT gender, AVG(month_salary) FROM Staff GROUP BY gender HAVING gender='{{gender}}'",
};
